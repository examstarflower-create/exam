...
int main(int argc, char **argv)
{
    ...
    if (autograder) {
		printf("correct:%d\n", numcorrect);
		printf("perfidx:%.0f\n", perfindex);
		printf("Final score: %.1f (correctness) + %.1f (performance) = %.1f\n", numcorrect*4.0, perfindex*56.0/100.0, numcorrect*4.0 + perfindex*56.0/100.0);
    }
...
static trace_t *read_trace(char *tracedir, char *filename)
{
	...
	sprintf(msg, "Could not open %.900s in read_trace", path);
	unix_error(msg);
    }
